...
sidr_data = genfromtxt('https://raw.githubusercontent.com/Patrick796/Patrick/main/a..csv', delimiter=',') #in the form of [t,S,I,D,R]
 ...
 #nets
    class Net_sidr(nn.Module): # input = [t]
        ...
        #R_t
            ...
            f2_hat = I_hat_t - (self.beta * S * I - self.mu * I - self.gamma * self.lamda * I - self.gamma * self.delta * I ) / (self.I_max - self.I_min)
            f3_hat = D_hat_t - (self.gamma * self.Lamda * I) / (self.D_max - self.D_min)
            f4_hat = R_hat_t - (self.gamma * self.delta * I -self.mu * R) / (self.R_max - self.R_min)
...
dinn = DINN(sidr_data[0], sidr_data[1], sidr_data[2], sidr_data[3], sidr_data[4]) #in the form of [t,S,I,D,R]
...
try:S_pred_list, I_pred_list, D_pred_list, R_pred_list = dinn.train(1000) #train
except EOFError:
    if dinn.save == 2:
        dinn.save = 3
...
ax.scatter(sidr_data[0], sidr_data[1], color = 'red', label='Susceptible Data', s=800)
...
ax.scatter(sidr_data[0], sidr_data[2], color = 'violet', label='Infected Data', s=800)
...
ax.scatter(sidr_data[0], sidr_data[3], color = 'black', label='Dead Data', s=800)
...
ax.scatter(sidr_data[0], sidr_data[4], color = 'darkorchid', label='Recovered Data', s=800)
ax.plot(sidr_data[0], R_pred_list[0].detach().numpy(), 'teal', alpha=0.9, lw=2, label='Recovered Prediction', linestyle='dashed')


ax.set_xlabel('Time /days',size = 800)
ax.set_ylabel('Number',size = 800)
#ax.set_ylim([-1,50])
...
legend = ax.legend(prop={'size':800})
